...
const NhaSanXuat = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: () => {
      handleCreateOrUpdateNhaSanXuat();
    }
  });
  ...
  const fetchDataNhaSanXuat = async () => {
    try {
      ...
      if (Object.keys(searchParams).length > 0) {
        response = await searchThuocTinhSanPham(searchParams.search, currentPage, pageSize, "nha-san-xuat");
      } else {
        response = await getAll(currentPage, pageSize, "nha-san-xuat");
      }
      setListNhaSanXuat(response.content);
      setTotalPages(response.totalPages);
      console.log(listNhaSanXuat);
    } catch (error) {
  ...
  // add / update
  const handleCreateOrUpdateNhaSanXuat = async () => {
    try {
      let status;
      if (formik.values.id) {
        status = await updateById(formik.values.id, "nha-san-xuat", formik.values);
      } else {
        status = await create("nha-san-xuat", formik.values);
  ...
  const handleRemove = async (post) => {
    try {
      await revertOrRemove(post.id, "nha-san-xuat", "remove");
      await fetchDataNhaSanXuat();
    } catch (error) {
      console.log("Error reverting NhaSanXuat:", error);
    }
  };

  const handleRevert = async (post) => {
    try {
      await revertOrRemove(post.id, "nha-san-xuat", "revert");
      await fetchDataNhaSanXuat();
    } catch (error) {
      console.log("Error revertting NhaSanXuat:", error);
    }
  };
  ...
  return (
    <MainCard title="Nhà Sản Xuất">
      <Grid container spacing={5} style={{marginBottom: "30px"}}>
        <Grid item xs={4}>
          <FormControl fullWidth size="small">
            <OutlinedInput
              id="input-search-header"
              placeholder="Tìm kiếm theo mã, tên nhà sản xuất"
              value={searchValue}
          ...
          <DialogContent>
            <Typography variant="body3">
              <h1 style={{textAlign: "center", marginBottom: "40px"}}>
                {formik.values?.id ? "Cập Nhật Nhà Sản Xuất" : "Thêm Nhà Sản Xuất"}
              </h1>
              <Grid fullWidth>
                <TextField
      ...
      <TableContainer component={Paper}>
        <Table sx={{marginBottom: "30px"}} aria-label="simple table">
          <TableHead>
            <TableRow>
              <TableCell>#</TableCell>
              <TableCell>Mã nhà sản xuất</TableCell>
