...
const LoaiSanPham = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: () => {
      handleCreateOrUpdateLoaiSanPham();
    }
  });
  ...
  const fetchDataLoaiSanPham = async () => {
    try {
      ...
      if (Object.keys(searchParams).length > 0) {
        response = await searchThuocTinhSanPham(searchParams.search, currentPage, pageSize, "loai-san-pham");
      } else {
        response = await getAll(currentPage, pageSize, "loai-san-pham");
      }
      setListLoaiSanPham(response.content);
      setTotalPages(response.totalPages);
      console.log(listLoaiSanPham);
    } catch (error) {
  ...
  // add / update
  const handleCreateOrUpdateLoaiSanPham = async () => {
    try {
      let status;
      if (formik.values.id) {
        status = await updateById(formik.values.id, "loai-san-pham", formik.values);
      } else {
        status = await create("loai-san-pham", formik.values);
  ...
  const handleRemove = async (post) => {
    try {
      await revertOrRemove(post.id, "loai-san-pham", "remove");
      await fetchDataLoaiSanPham();
    } catch (error) {
      console.log("Error reverting LoaiSanPham:", error);
    }
  };

  const handleRevert = async (post) => {
    try {
      await revertOrRemove(post.id, "loai-san-pham", "revert");
      await fetchDataLoaiSanPham();
    } catch (error) {
      console.log("Error revertting LoaiSanPham:", error);
    }
  };
  ...
  return (
    <MainCard title="Loại Sản Phẩm">
      <Grid container spacing={5} style={{marginBottom: "30px"}}>
        <Grid item xs={4}>
          <FormControl fullWidth size="small">
            <OutlinedInput
              id="input-search-header"
              placeholder="Tìm kiếm theo mã, tên loại sản phẩm"
              value={searchValue}
          ...
          <DialogContent>
            <Typography variant="body3">
              <h1 style={{textAlign: "center", marginBottom: "40px"}}>
                {formik.values?.id ? "Cập Nhật Loại Sản Phẩm" : "Thêm Loại Sản Phẩm"}
              </h1>
              <Grid fullWidth>
                <TextField
