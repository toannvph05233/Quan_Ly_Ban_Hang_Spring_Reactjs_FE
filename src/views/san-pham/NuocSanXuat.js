...
const NuocSanXuat = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: () => {
      handleCreateOrUpdateNuocSanXuat();
    }
  });
  ...
  const fetchDataNuocSanXuat = async () => {
    try {
      ...
      if (Object.keys(searchParams).length > 0) {
        response = await searchThuocTinhSanPham(searchParams.search, currentPage, pageSize, "nuoc-san-xuat");
      } else {
        response = await getAll(currentPage, pageSize, "nuoc-san-xuat");
      }
      setListNuocSanXuat(response.content);
      setTotalPages(response.totalPages);
      console.log(listNuocSanXuat);
    } catch (error) {
  ...
  // add / update
  const handleCreateOrUpdateNuocSanXuat = async () => {
    try {
      let status;
      if (formik.values.id) {
        status = await updateById(formik.values.id, "nuoc-san-xuat", formik.values);
      } else {
        status = await create("nuoc-san-xuat", formik.values);
  ...
  const handleRemove = async (post) => {
    try {
      await revertOrRemove(post.id, "nuoc-san-xuat", "remove");
      await fetchDataNuocSanXuat();
    } catch (error) {
      console.log("Error reverting NuocSanXuat:", error);
    }
  };

  const handleRevert = async (post) => {
    try {
      await revertOrRemove(post.id, "nuoc-san-xuat", "revert");
      await fetchDataNuocSanXuat();
    } catch (error) {
      console.log("Error revertting NuocSanXuat:", error);
    }
  };
  ...
  return (
    <MainCard title="Nước Sản Xuất">
      <Grid container spacing={5} style={{marginBottom: "30px"}}>
        <Grid item xs={4}>
          <FormControl fullWidth size="small">
            <OutlinedInput
              id="input-search-header"
              placeholder="Tìm kiếm theo mã, tên nước sản xuất"
              value={searchValue}
          ...
          <DialogContent>
            <Typography variant="body3">
              <h1 style={{textAlign: "center", marginBottom: "40px"}}>
                {formik.values?.id ? "Cập Nhật Nước Sản Xuất" : "Thêm Nước Sản Xuất"}
              </h1>
              <Grid fullWidth>
                <TextField
      ...
      <TableContainer component={Paper}>
        <Table sx={{marginBottom: "30px"}} aria-label="simple table">
          <TableHead>
            <TableRow>
              <TableCell>#</TableCell>
              <TableCell>Mã nước sản xuất</TableCell>
              <TableCell>Tên nước sản xuất</TableCell>
              <TableCell>Trạng thái</TableCell>
              <TableCell>Hoạt động</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {listNuocSanXuat.map((row, index) => (
              <TableRow key={row.id}>
