...
const DongSanPham = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: () => {
      handleCreateOrUpdateDongSanPham();
    }
  });
  ...
  const fetchDataDongSanPham = async () => {
    try {
      ...
      if (Object.keys(searchParams).length > 0) {
        response = await searchThuocTinhSanPham(searchParams.search, currentPage, pageSize, "dong-san-pham");
      } else {
        response = await getAll(currentPage, pageSize, "dong-san-pham");
      }
      setListDongSanPham(response.content);
      setTotalPages(response.totalPages);
      console.log(listDongSanPham);
    } catch (error) {
  ...
  // add / update
  const handleCreateOrUpdateDongSanPham = async () => {
    try {
      let status;
      if (formik.values.id) {
        status = await updateById(formik.values.id, "dong-san-pham", formik.values);
      } else {
        status = await create("dong-san-pham", formik.values);
  ...
  const handleRemove = async (post) => {
    try {
      await revertOrRemove(post.id, "dong-san-pham", "remove");
      await fetchDataDongSanPham();
    } catch (error) {
      console.log("Error reverting DongSanPham:", error);
    }
  };

  const handleRevert = async (post) => {
    try {
      await revertOrRemove(post.id, "dong-san-pham", "revert");
      await fetchDataDongSanPham();
    } catch (error) {
      console.log("Error revertting DongSanPham:", error);
    }
  };
  ...
  return (
    <MainCard title="Dòng Sản Phẩm">
      <Grid container spacing={5} style={{marginBottom: "30px"}}>
        <Grid item xs={4}>
          <FormControl fullWidth size="small">
            <OutlinedInput
              id="input-search-header"
              placeholder="Tìm kiếm theo mã, tên dòng sản phẩm"
              value={searchValue}
          ...
          <DialogContent>
            <Typography variant="body3">
              <h1 style={{textAlign: "center", marginBottom: "40px"}}>
                {formik.values?.id ? "Cập Nhật Dòng Sản Phẩm" : "Thêm Dòng Sản Phẩm"}
              </h1>
              <Grid fullWidth>
                <TextField
      ...
      <TableContainer component={Paper}>
        <Table sx={{marginBottom: "30px"}} aria-label="simple table">
          <TableHead>
            <TableRow>
              <TableCell>#</TableCell>
              <TableCell>Mã dòng sản phẩm</TableCell>
              <TableCell>Tên dòng sản phẩm</TableCell>
              <TableCell>Trạng thái</TableCell>
              <TableCell>Hoạt động</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {listDongSanPham.map((row, index) => (
              <TableRow key={row.id}>
