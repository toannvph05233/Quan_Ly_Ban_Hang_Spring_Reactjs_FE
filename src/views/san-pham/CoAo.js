...
const CoAo = () => {
  ...
  const fetchDataCoAo = async () => {
    try {
      ...
      if (Object.keys(searchParams).length > 0) {
        response = await searchThuocTinhSanPham(searchParams.search, currentPage, pageSize, "co-ao");
      } else {
        response = await getAll(currentPage, pageSize, "co-ao");
      }
      setListCoAo(response.content);
  ...
  // add / update
  const handleCreateOrUpdateCoAo = async () => {
    try {
      let status;
      if (formik.values.id) {
        status = await updateById(formik.values.id, "co-ao", formik.values);
      } else {
        status = await create("co-ao", formik.values);
  ...
  const handleRemove = async (post) => {
    try {
      await revertOrRemove(post.id, "co-ao", "remove");
      await fetchDataCoAo();
    } catch (error) {
      console.log("Error reverting CoAo:", error);
    }
  };

  const handleRevert = async (post) => {
    try {
      await revertOrRemove(post.id, "co-ao", "revert");
      await fetchDataCoAo();
    } catch (error) {
      console.log("Error revertting CoAo:", error);
    }
  };
  ...
  return (
    <MainCard title="Cổ Áo">
      <Grid container spacing={5} style={{marginBottom: "30px"}}>
        <Grid item xs={4}>
          <FormControl fullWidth size="small">
            <OutlinedInput
              id="input-search-header"
              placeholder="Tìm kiếm theo mã, tên cổ áo"
              value={searchValue}
        ...
        <form onSubmit={formik.handleSubmit}>
          <DialogContent>
            <Typography variant="body3">
              <h1 style={{textAlign: "center", marginBottom: "40px"}}>{formik.values?.id ? "Cập Nhật Cổ Áo" : "Thêm Cổ Áo"}</h1>
              <Grid fullWidth>
                <TextField
                  label="Tên cổ áo"
                  name="ten"
          ...
          <DialogActions>
            <Button type="submit" variant="contained">
              {formik.values?.id ? "Cập Nhật Cổ Áo" : "Thêm Cổ Áo"}
            </Button>
      ...
      <TableContainer component={Paper}>
        <Table sx={{marginBottom: "30px"}} aria-label="simple table">
          <TableHead>
            <TableRow>
              <TableCell>#</TableCell>
              <TableCell>Mã cổ áo</TableCell>
              <TableCell>Tên cổ áo</TableCell>
              <TableCell>Trạng thái</TableCell>
              <TableCell>Hoạt động</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {listCoAo.map((row, index) => (
              <TableRow key={row.id}>
