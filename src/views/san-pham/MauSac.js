...
const MauSac = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ma: Yup.string().min(5, "Mã phải lớn hơn 5 kí tự").max(225, "Mã không dài quá 225 kí tự").required("Mã không được để trống")
    }),
    onSubmit: () => {
      handleCreateOrUpdateMauSac();
    }
  });
  ...
  const fetchDataMauSac = async () => {
    try {
      ...
      if (Object.keys(searchParams).length > 0) {
        response = await searchThuocTinhSanPham(searchParams.search, currentPage, pageSize, "mau-sac");
      } else {
        response = await getAll(currentPage, pageSize, "mau-sac");
      }
      setListMauSac(response.content);
  ...
  // add / update
  const handleCreateOrUpdateMauSac = async () => {
    try {
      let status;
      if (formik.values.id) {
        status = await updateById(formik.values.id, "mau-sac", formik.values);
      } else {
        status = await create("mau-sac", formik.values);
  ...
  const handleRemove = async (post) => {
    try {
      await revertOrRemove(post.id, "mau-sac", "remove");
      await fetchDataMauSac();
    } catch (error) {
      console.log("Error reverting MauSac:", error);
    }
  };

  const handleRevert = async (post) => {
    try {
      await revertOrRemove(post.id, "mau-sac", "revert");
      await fetchDataMauSac();
    } catch (error) {
      console.log("Error revertting MauSac:", error);
    }
  };
  ...
  return (
    <MainCard title="Màu Sắc">
      <Grid container spacing={5} style={{marginBottom: "30px"}}>
        <Grid item xs={4}>
          <FormControl fullWidth size="small">
            <OutlinedInput
              id="input-search-header"
              placeholder="Tìm kiếm theo mã màu sắc"
              value={searchValue}
        ...
        <form onSubmit={formik.handleSubmit}>
          <DialogContent>
            <Typography variant="body3">
              <h1 style={{textAlign: "center", marginBottom: "40px"}}>{formik.values?.id ? "Cập Nhật Màu Sắc" : "Thêm Màu Sắc"}</h1>
              <Grid fullWidth>
                <TextField
                  label="Mã màu sắc"
                  name="ma"
                ...
                {formik.errors.ma && formik.touched.ma && (
                  <Typography style={{color: "red", marginBottom: "13px"}}>{formik.errors.ma}</Typography>
                )}
              </Grid>
            </Typography>
          </DialogContent>
          <DialogActions>
            <Button type="submit" variant="contained">
              {formik.values?.id ? "Cập Nhật Màu Sắc" : "Thêm Màu Sắc"}
            </Button>
